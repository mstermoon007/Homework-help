...
(function (global) {
  ...
  /**
   * 统一 PracticeRequest → practice.html URL（R02 / R03）。
   * 全站唯一 URL 拼接入口；旧深链参数（kp / kps / qt / types / book / count / difficulty / adaptiveMode）一律兼容。
   * 数组参数统一 csv：kps=kp1,kp2  qt=<主类型>  types=<csv>
   */
  function buildPracticeUrl(request) {
    if (!request || typeof request !== 'object') return 'practice.html';
    var subj = (request.subject || 'math');
    var grade = (request.grade != null) ? request.grade : 1;
    var parts = ['practice.html?subject=' + encodeURIComponent(subj) + '&grade=' + grade];

    if (request.mode) parts.push('mode=' + encodeURIComponent(request.mode));

    // KP 池：优先 scope.knowledgePoints，兼容顶层 knowledgePoints / knowledgePointIds / kp
    var kps = (request.scope && Array.isArray(request.scope.knowledgePoints) && request.scope.knowledgePoints.length)
      ? request.scope.knowledgePoints
      : (Array.isArray(request.knowledgePoints) ? request.knowledgePoints
        : (Array.isArray(request.knowledgePointIds) ? request.knowledgePointIds : null));
    if (typeof kps === 'string') kps = kps.split(',').filter(Boolean);
    if (kps && kps.length) {
      parts.push('kps=' + encodeURIComponent(kps.join(',')));
    } else if (request.knowledgePointId || request.kp) {
      parts.push('kp=' + encodeURIComponent(request.knowledgePointId || request.kp));
    }

    // 题型：兼容 questionTypes[]（规范）与顶层 questionType
    var RN = (typeof window !== 'undefined' && window.RequestNormalize) || null;
    var qts = Array.isArray(request.questionTypes) ? request.questionTypes.slice()
      : (request.questionType ? [request.questionType] : []);
    if (RN) qts = RN.normalizeQuestionTypes(qts);
    if (qts.length) {
      parts.push('qt=' + encodeURIComponent(qts[0]));
      parts.push('types=' + encodeURIComponent(qts.join(',')));
    } else if (request.questionType) {
      parts.push('qt=' + encodeURIComponent(request.questionType));
    }

    if (request.count != null) parts.push('count=' + request.count);
    if (request.difficulty && request.difficulty !== 'normal') parts.push('difficulty=' + encodeURIComponent(request.difficulty));
    if (request.adaptiveMode) parts.push('adaptiveMode=new');

    var book = (request.scope && request.scope.book) || request.book;
    if (book && book !== 'all') parts.push('book=' + encodeURIComponent(book));
   ...
   * 现委托 buildPracticeUrl（统一入口，避免重复拼接）。
   */
  function buildPracticeLink(subject, grade, moduleId, questionType) {
    ...
    var request = {
      subject: subject, grade: grade, mode: (kpIds.length > 1 ? 'multi-kp' : 'single-kp'),
      knowledgePoints: kpIds, questionTypes: [questionType]
    };
    return buildPracticeUrl(request);
  }
