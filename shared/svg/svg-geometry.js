// shared/svg/svg-geometry.js — 1~6 年级常见几何图形生成器（SVGGeometry）
//
// 依赖 shared/svg/svg-core.js（须先加载），输出完整 <svg> 字符串，可直接赋给题目 q.svg。
...
(function (global) {
  ...
  if (!U) throw new Error('shared/svg/svg-geometry.js 依赖 shared/svg/svg-core.js，请先加载');
  ...
  // ============ 位置关系网格（position 生成器 position-grid） ============
  /** 网格场景：{gridSize, unitPx, objects:[{name,x,y}], showGrid}=网格 + 物体圆点 + 名称标注 */
  function positionGrid(o) {
    var n = o.gridSize || 5;
    var up = o.unitPx || 35;
    var w = n * up, h = n * up;
    var inner = '';
    if (o.showGrid !== false) {
      var gridColor = o.gridColor || '#d8dee9';
      for (var g = 1; g < n; g++) {
        inner += U.svgLine(0, g * up, w, g * up, { stroke: gridColor, strokeWidth: 1 });
        inner += U.svgLine(g * up, 0, g * up, h, { stroke: gridColor, strokeWidth: 1 });
      }
    }
    var colors = ['#3f6fd1', '#e05252', '#2f9e6b', '#c77f2a', '#7c5cff', '#d9822b', '#1f9d8a', '#b548c0'];
    (o.objects || []).forEach(function (obj, i) {
      if (!obj || obj.x == null || obj.y == null) return;
      var cx = (obj.x + 0.5) * up, cy = (obj.y + 0.5) * up;
      var col = colors[i % colors.length];
      inner += U.svgCircle(cx, cy, 9, { fill: col, stroke: '#ffffff', strokeWidth: 2 });
      if (obj.name) {
        inner += U.svgText(cx, cy - 15, obj.name, { fontSize: 12, fill: '#27324a', 'text-anchor': 'middle' });
      }
    });
