/**
 * shared/core/common.js — 共享基础模块（站点 + 插件，统一入口 · 任务 3.2 聚合出口）
 *
 ...
 * Node：经 require 加载子模块并导出 PluginUtil（保持 `const PluginUtil = require('./shared/core/common.js')` 契约不变）。
 *
 * 浏览器：<script src="shared/core/common.js"></script>
 *   全局 App：页面路由、年级参数、页面控制器
 *   全局 PluginUtil：插件随机/标准化工具（插件内不得直接用 Math.random()）
 * Node：const PluginUtil = require('./shared/core/common.js')
 */
(function (global) {
  'use strict';
