/**
 * shared/knowledge/question-type-registry.js — 全局唯一题型注册表 (M2-01 / M2-02 / M2-03)
 *
 * 用户侧规范题型仅 7 类（知识点驱动生成，题型由知识点 applicable_question_types 决定）：
 *   calc         计算题（吸收 口算 oral）
 *   fill         填空题
 *   choice       选择题
 *   judge        判断题
 *   geometry     操作/作图题（吸收 认读 recognize）
 *   classify     分类整理题（新增）
 *   apply        解决问题/应用题（吸收 开放 open）
 * 竞赛（competition）为「模式/标签」而非独立题型，落点归 apply。
 *
 * 归并策略（确定性、可审计）：
 *   1) 精确匹配 7 类规范 ID
 *   2) 显式 canonical 别名（operate->calc / recognize->geometry / open->apply / classify 独立）
 *   3) 关键字启发式：几何关键字 -> geometry；认读关键字 -> geometry；其余 -> calc
...
(function (global) {
  ...
  // 规范题型（用户侧仅 7 类；竞赛为模式标签，不单列题型）。
  // 归并：口算→calc；认读→geometry；开放→apply；分类整理为独立 classify。
  var TYPES = [
  ...
  // 生成模式（竞赛为标签，不单列题型）。快速=按单元直出题型；教师=按知识点选题型；竞赛=按题型标签。
  ...
  var API = {
    ...
    get: function (id) {
      ...
      // 别名解析：legacy 题型 token（oral/open/recognize）仍可在展示层取到规范题型元信息
