/**
 * shared/knowledge/knowledge-bank.js — 全科目知识库（按科目分组，1-6 年级）
 *
 * 数据结构（任务3 按科目分组；本文件为「入口壳」——仅含空组占位、查询方法与分片映射表，
 * 实际数据在 shared/knowledge/knowledge-math.js 分片中，加载/装配顺序见下）：
 *   KnowledgeBank = {
 *     math: [ { grade: 1, modules: [...] }, ... ]   // ← 由 knowledge-math.js 装配
 *   }
 *
 * 加载与装配：
 *   - 浏览器：<script src="shared/knowledge/knowledge-bank.js">（入口，先）→ 按科目经
 ...
 *       modules: [
 *         {
 *           moduleId: 'M0',                 // 对应 shared/catalog/module-catalog.js 中的题型模块 ID
 *           knowledgePoints: [
 ...
 *     }
 *
 * 浏览器：<script src="shared/knowledge/knowledge-bank.js"></script> -> 全局 KnowledgeBank（对象）
 * Node：  const KnowledgeBank = require('./shared/knowledge/knowledge-bank.js')
 *
...
(function (global) {
  ...
  KnowledgeBank.SHARDS = {
    math: 'shared/knowledge/knowledge-math.js'
  };
