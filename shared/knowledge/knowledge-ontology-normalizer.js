/**
 * shared/knowledge/knowledge-ontology-normalizer.js — Legacy → Canonical 归一化 (M1-01)
 *
 * 纯数据转换：不依赖 DOM / window / 插件 / UI / 题目生成。
 * 同一个 Legacy KP 输入必须得到相同 Canonical 输出（可重复）。
 * 不修改原对象；原始信息通过 source / legacy 字段保留。
 *
 * 映射严格依据 shared/catalog/difficulty-static.js 既有的字段语义
...
(function (global) {
  ...
  var MODULE_CATALOG = (function () {
    try { return require('../catalog/module-catalog.js'); } catch (e) { return null; }
  })();
  ...
  function canonQuestionType(t) {
    ...
    // 7 类规范题型改造：经 question-type-registry 别名归一（oral→calc / recognize→geometry / open→apply）
