/**
 * shared/engine/practice-session.js — P7-R01/P7-R02/P7-R03 统一练习会话
 *
 * 统一练习流程：
...
(function (global) {
  ...
  function PracticeSession(options) {
    ...
    this.config = {
      ...
      perTypeCount: options.perTypeCount != null ? options.perTypeCount : null,
      typeCounts: Array.isArray(options.typeCounts) ? options.typeCounts : null,
  ...
  PracticeSession.prototype._buildGenerationRequest = function () {
    ...
    if (this.config.knowledgePointIds && this.config.knowledgePointIds.length) {
      ...
      req.mode = 'multi-kp';
      // 双量控制：总数量 count + 分题型数量（perTypeCount 统一值 / typeCounts 逐题型），
      // 由生成层决策（planByType）按题型选择最优知识点群、群内均分。旧 kpAllocation 配额已废除。
      if (this.config.perTypeCount != null) req.perTypeCount = this.config.perTypeCount;
      if (this.config.typeCounts && this.config.typeCounts.length) req.typeCounts = this.config.typeCounts;
