/**
 * shared/engine/generation-engine.js — 生成层引擎（统一生成最终入口，M7-R08 主链精简版）
 *
 * 主链（R08 精简）：
 ...
 * 内部构成：
 *   GenerationEngine
 *    ├── KnowledgeResolver    → shared/generator/registry-facade.js（capability 解析，不知具体插件）
 ...
 *    ├── GeneratorRegistry    → shared/generator/registry-facade.js / generator/generator-registry.js
 *    ├── Validator            → shared/validator/validation-pipeline.js（核心四项，Retry/Regenerate 内建）
 *    └── SemanticQuestionNormalizer → shared/semantic/semantic-question.js（工厂规范化）
...
(function (global) {
  ...
  function getGeneratorRegistry() {
    return ensure(null, 'GeneratorRegistry', '../generator/registry-facade.js');
  }

  // R15：内部构成说明（只读）
  var PIPELINE = {
    KnowledgeResolver: 'shared/generator/registry-facade.js',
    StrategyEngine: 'shared/strategy/strategy-engine.js',
    QuestionPlanner: 'shared/strategy/comprehensive-strategy.js',
    GeneratorRegistry: 'shared/generator/registry-facade.js',
    Validator: 'shared/validator/validation-pipeline.js',
    SemanticQuestionNormalizer: 'shared/semantic/semantic-question.js'
  };
  ...
  function requestKpIds(request) {
    ...
    if (Array.isArray(request.knowledgePointIds) && request.knowledgePointIds.length) return request.knowledgePointIds.slice();
    if (Array.isArray(request.knowledgePoints) && request.knowledgePoints.length) return request.knowledgePoints.slice();
  ...
  function build(request) {
    ...
    // multi-kp：由生成层决策传入 knowledgePointIds + 题型 + 数量（总数量/分题型数量）规划 plans
    ...
    if (request.combine === true && kpList.length > 1) {
      if (!engine) return Promise.reject(new Error('StrategyEngine 不可用，请先加载 shared/engine/strategy-engine.bundle.js'));
      try {
        var combinedResult = engine.plan(request);
    ...
    if (mode === 'multi-kp' || kpList.length > 1) {
      ...
      try {
        var typeResult = engine.planByType(request);
   ...
   */
  /**
   * R2/R5：全局题量预算（委托 GenerationAPI.generateBudget）
   * @param {Object} request
   * @param {Object} [options]
   * @returns {Promise<Object>}
   */
  function generateBudget(request, options) {
    if (GenerationAPI && typeof GenerationAPI.generateBudget === 'function') {
      return GenerationAPI.generateBudget(request, options);
    }
    throw new Error('GenerationAPI.generateBudget 不可用');
  }
