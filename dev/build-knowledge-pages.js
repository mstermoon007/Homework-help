...
function pageHtml(kp, relations) {
  var v = kpView(kp);
  var g = v.grade;
  var title = GRADE_CN[g] + '数学 · ' + (v.unitName || '') + ' · ' + v.name + ' · Homework Help 知识库';
  var desc = (v.concept || v.description || v.name) + '——' + GRADE_CN[g] + '数学知识点说明、典型例题与在线练习入口。';
  var kws = [];
  ...
  var cards = '';
  cards += '<div class="card"><h2>知识点说明</h2><p>' + esc(v.concept || v.description || '') + '</p>'
    + (kws.length ? '<div>' + kws.map(function (k) { return '<span class="kw">' + esc(k) + '</span>'; }).join('') + '</div>' : '') + '</div>';
  if (v.example) {
    cards += '<div class="card"><h2>典型例题</h2><p>' + esc(v.example) + '</p></div>';
  }
  if (v.qts.length) {
    cards += '<div class="card"><h2>可练题型</h2><div>' + v.qts.map(function (q) { return '<span class="kw">' + esc(q) + '</span>'; }).join('') + '</div></div>';
  }
  if (relations.prereq.length || relations.successor.length) {
    ...
    cards += '<div class="card"><h2>相关知识点</h2><ul class="list">' + relHtml + '</ul></div>';
  }
  cards += '<div class="card"><h2>在线练习</h2><p>点击下方按钮，在 Homework Help 练习页实时生成「' + esc(v.name) + '」相关题目，支持在线作答与即时批改。</p>'
    + '<a class="btn" href="../practice.html?subject=math&grade=' + encodeURIComponent(g) + '&kps=' + encodeURIComponent(v.id) + '">去练习：' + esc(v.name) + '</a>'
    + '<a class="btn ghost" href="knowledge-index.html">返回知识库首页</a></div>';

  var html = '<!DOCTYPE html>\n<html lang="zh-CN">\n<head>\n<meta charset="UTF-8">\n'
    + '<meta name="viewport" content="width=device-width, initial-scale=1.0">\n'
    + '<title>' + esc(title) + '</title>\n'
    + '<meta name="description" content="' + esc(desc) + '">\n'
    + '<link rel="canonical" href="https://home.modouyu.top/knowledge/' + esc(v.id) + '.html">\n'
  ...
  var hash = sha256(JSON.stringify({ v: TEMPLATE_VERSION, kp: v, rel: relations }));
  return html + '\n<!-- kbgen:hash=' + hash + ' -->\n';
}
...
function indexHtml(byGrade, total) {
  ...
  Object.keys(byGrade).sort().forEach(function (g) {
    ...
    Object.keys(byUnit).forEach(function (unit) {
      sections += '<div class="card"><h2 style="font-size:16px;">' + esc(unit) + '</h2><ul class="list" style="display:flex;flex-wrap:wrap;gap:0;">';
  ...
  var html = '<!DOCTYPE html>\n<html lang="zh-CN">\n<head>\n<meta charset="UTF-8">\n'
    ...
    + '<link rel="canonical" href="https://home.modouyu.top/knowledge/knowledge-index.html">\n'
    + '<link rel="stylesheet" href="../shared/styles/tokens.css">\n<style>' + CSS + '</style>\n</head>\n<body>'
    + '<nav class="crumb"><span>知识库首页</span></nav><h1>小学数学知识库</h1>'
    + '<div class="meta">覆盖 1–6 年级 · 共 ' + total + ' 个可练知识点</div>'
    + '<div class="card"><a class="btn" href="../index.html">返回 Homework Help 首页</a> <a class="btn ghost" href="../select.html">去选择练习</a></div>'
    + sections
    + '<footer>Homework Help · 免费无广告小学 1-6 年级家庭作业生成器 · 纯前端</footer>\n</body>\n</html>';
...
function main() {
  ...
  if (!checkOnly) {
    byGrade[1].forEach(function () {}); // noop
    Object.keys(byGrade).forEach(function (g) {
      byGrade[g].forEach(function (kp) {
        ...
        var html = pageHtml(kp, { prereq: prereq, successor: successor });
