...
(function (global) {
  ...
  function cardHTML(idx, inner, tag) {
    return '<div class="question-card" data-index="' + idx + '" style="border:1px solid #e3e9f2;border-radius:14px;padding:14px 0.5cm;position:relative;background:#fff;box-shadow:0 8px 24px rgba(40,70,120,.08);">' +
      ...
      '&nbsp;&nbsp;&nbsp;&nbsp;' +
      '<span class="q-text" style="font-size:12px;color:#7a879c;font-weight:700;display:inline;vertical-align:middle;">' + (tag || '计算下面各题') + '</span>' +
      '</div>' +
      inner +
  ...
  function qRender(q, idx) {
    ...
    return cardHTML(idx, '<div style="font-size:15px;font-weight:800;color:#27324a;line-height:1.6;">' + q.q + '</div><div style="margin-top:6px;">' + singleInp(idx) + '</div>', '笔算/脱式');
  }
