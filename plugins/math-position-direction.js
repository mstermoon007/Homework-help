...
(function (global) {
  ...
  function buildCompass() {
    var items = [
      ...
      { q: '晚上面对北极星，我的前面是哪个方向？', a: '北' },
      { q: '树的年轮较疏的一面向着哪个方向？', a: '南' },
      { q: '树的年轮较密的一面向着哪个方向？', a: '北' },
      { q: '冬天，沟渠里积雪融化较快的一面是？', a: '南' },
      { q: '指南针红色指针总是指向哪个方向？', a: '北' },
      { q: '地图通常是按上（ ）下南、左西、右东绘制的，括号里填哪个方向？', a: '北' },
      { q: '面向南，我的后面是哪个方向？', a: '北' },
      { q: '面向南，我的左手边是哪个方向？', a: '东' },
      { q: '面向南，我的右手边是哪个方向？', a: '西' },
      { q: '面向西，我的后面是哪个方向？', a: '东' },
      { q: '面向西，我的左手边是哪个方向？', a: '南' },
      { q: '面向西，我的右手边是哪个方向？', a: '北' },
      { q: '东风是从哪个方向吹来的风？', a: '东' },
      { q: '南风是从哪个方向吹来的风？', a: '南' }
    ];
    var it = pick(items);
  ...
  // 方向旋转：面向某方向，左/右转 90°（可多次）面向哪里
  function buildTurn() {
    var facing = pick(DIRS);
    var turn = pick(['左', '右']);
    var k = rnd(1, 3); // 转 1~3 次，每次 90°
    // 顺时针：东→南→西→北；逆时针反之
    var CW = ['东', '南', '西', '北'];
    var idx = CW.indexOf(facing);
    var step = turn === '右' ? 1 : 3;
    var target = CW[(idx + step * k) % 4];
    var q = '面向' + facing + '，向' + turn + '转' + (k === 1 ? ' 90°' : (' ' + k + ' 次（每次 90°）')) + '，最后面向哪个方向？';
    return {
      kind: 'turn',
      question: q,
      answer: target,
