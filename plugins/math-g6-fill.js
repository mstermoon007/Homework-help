...
(function (global) {
  ...
  function buildNegative() {
    ...
    return { q: '0 既不是正数，也不是（  ）（如温度 ' + rnd(2, 9) + '℃ 和 −' + rnd(2, 9) + '℃ 都以 0 为分界）', answer: '负数', hint: '0 是正数和负数的分界点。' };
  }
  ...
  function buildPercent() {
    ...
    if (v === 'discount') {
      var pair = pick([[65, '六五'], [85, '八五'], [90, '九'], [75, '七五'], [80, '八'], [50, '五'], [70, '七'], [95, '九五'], [60, '六'], [88, '八八']]);
      return { q: '一件衣服按 ' + pair[1] + '折出售，' + pair[1] + '折写成百分数是（  ）%', answer: pair[0], hint: '几几折就是百分之几十几。' };
    }
    if (v === 'toPct') {
      var tenths = pick([11, 12, 15, 18, 22, 25, 28, 33, 35, 38, 42, 45, 48, 52, 55, 58, 62, 64, 68, 72, 75, 78, 82, 85, 88, 92, 95, 98]);
      var dec = tenths / 100;
      return { q: trimD(dec) + ' =（  ）%（如 ' + trimD(dec) + ' 米 = ' + tenths + ' 厘米）', answer: tenths + '%', hint: '小数化成百分数：小数点向右移动两位，添上百分号。' };
    }
    if (v === 'meaning') {
      var pc = rnd(10, 99);
      return { q: '六（' + rnd(1, 9) + '）班有 ' + pc + '% 的同学参加了兴趣小组，这里的 ' + pc + '% 表示（  ）', answer: '一个数是另一个数的百分之几', hint: '百分数也叫百分率或百分比，表示一个数是另一个数的百分之几。' };
    }
  ...
  function buildRatio() {
    ...
    if (v === 'basic') {
      var ba = rnd(2, 9), bb = rnd(2, 9), bk = rnd(2, 6);
      return { q: '如 ' + ba + ' : ' + bb + ' = (' + ba + ' × ' + bk + ') : (' + bb + ' × ' + bk + ')，比的前项和后项同时乘或除以相同的数（0 除外），比值（  ）', answer: '不变', hint: '比的基本性质。' };
    }
    if (v === 'prop') {
      var pa = rnd(2, 6), pb = rnd(2, 9), pcx = rnd(2, 9);
      return { q: '在比例 ' + pa + ' : ' + pb + ' = ' + (pa * pcx) + ' : ' + (pb * pcx) + ' 里，两个内项的积等于两个外项的积，这叫做比例的（  ）', answer: '基本性质', hint: '解比例的依据。' };
    }
  ...
  function buildCircle() {
    ...
    var rc = rnd(2, 12);
    var ex = (pair[1] === 'C=2πr') ? ('（如 r=' + rc + ' 时，C=2×3.14×' + rc + '）') :
             (pair[1] === 'S=πr²') ? ('（如 r=' + rc + ' 时，S=3.14×' + rc + '²）') :
             ('（如一个圆周长 ' + trimD(2 * PI3 * rc) + '、直径 ' + (2 * rc) + '，π = 周长 ÷ 直径）');
    return { q: pair[0] + ex, answer: pair[1], hint: '熟记圆的两个基本公式。' };
  }
  ...
  function buildPieChart() {
    ...
    if (v === 'relation') {
      var parts = rnd(3, 6);
      return { q: '六（' + rnd(1, 9) + '）班同学最喜欢的运动项目分 ' + parts + ' 类，用扇形统计图可以清楚地表示各部分数量与（  ）之间的关系', answer: '总数', hint: '扇形统计图反映部分占整体的百分比。' };
    }
    if (v === 'sum') {
      var ps = rnd(3, 6);
      var names = ['足球', '篮球', '跳绳', '唱歌', '绘画', '阅读'];
      var ex2 = [];
      for (var ii = 0; ii < ps; ii++) ex2.push(names[rnd(0, names.length - 1)]);
      return { q: '扇形统计图中，喜欢' + ex2.join('、') + '等 ' + ps + ' 类项目的各部分百分比之和是（  ）%', answer: 100, hint: '想一想：整个圆表示总数，各部分百分比合起来应占多少。' };
    }
    var big = pick(['红色', '黄色', '蓝色']);
    return { q: '扇形统计图中，' + big + '扇形面积越大，所占的（  ）越大', answer: '百分比', hint: '扇形大小与所占百分比对应。' };
  }
  ...
  var UNIT_ITEMS = [
    ...
    [['0.4', '米', '厘米'], 40, '1 米 = 100 厘米'],
    [['3.05', '千克', '克'], 3050, '1 千克 = 1000 克'],
    [['0.25', '公顷', '平方米'], 2500, '1 公顷 = 10000 平方米'],
    [['5.6', '平方千米', '公顷'], 560, '1 平方千米 = 100 公顷'],
    [['0.08', '吨', '千克'], 80, '1 吨 = 1000 千克'],
    [['2.5', '时', '分'], 150, '1 时 = 60 分'],
    [['1.8', '立方米', '立方分米'], 1800, '1 立方米 = 1000 立方分米']
  ];
