...
>
> - **唯一强制点**：运行时加载器 `practice.html` 的 `loadPlugin` 在脚本 `onload` 后校验插件对象必须同时含 `generate` / `render` / `check`，否则报「插件接口不兼容」并拒绝加载。**不存在 `PluginRegistry` 注册类**（旧文档中的 `PluginRegistry.register/verify` 已不再使用）。
...
插件文件（如 `plugins/math-oral.js`）在加载时把自身对象挂到全局 `window.__currentPlugin`。该对象由 **元数据 + 三大接口** 组成。
...
| `printConfig` | `{ pageType: string, title?: string }` | 打印配置。`pageType` 缺失时容器回退 `'math'`，用于打印模板分支；`title` 仅部分插件（如英语字母）使用 |
| `settings` | `array` | 题型子类别筛选 UI：`[{ key, label?, options: [{label, value}], default? }]`，`practice.html` 据此渲染题型切换 chip |
...
- **可同步也可异步**：可直接返回 `ExerciseSet`，也可返回 `Promise<ExerciseSet>`（异步插件，如 `math-comprehensive` 需先加载子插件再生成；`practice.html` 已处理 Promise）。
...
- **返回**：`{ score: number, total: number, correct: number, message: string, results: boolean[], correctAnswers: string[] }`
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `answer` | 是 | 标准答案，类型 `string \| number \| Array<string\|number>`，非空 |
...
- `answer` 必须为可比较的标准值；`check` 可自定义容错比较（如拼音声调、凑十法多输入）。
