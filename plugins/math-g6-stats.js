...
(function (global) {
  ...
  function buildPieChart() {
    ...
    var scope = pick(['全校', '全年级', '六年级', '三年级', '学校']);
    if (v === 'total') {
      var total = rnd(2, 12) * 100;
      var p = pick([5, 10, 15, 20, 25, 30, 40, 45, 50, 60]);
  ...
  function buildPossibility() {
    var v = pick(['coinBin', 'ball', 'even', 'certain', 'draw', 'dice', 'spinner', 'diceLt', 'diceGt', 'bag3']);
    if (v === 'coinBin') {
      var n = rnd(2, 6);
      var m = rnd(0, n);
    ...
    if (v === 'even') {
      var ne = 2 * rnd(3, 12);
      return { q: '从 1 到 ' + ne + ' 中任意抽取一个数，抽到偶数的可能性是（  ）', answer: '1/2', options: shuffle(['1/2', '1/3', '2/3', '1/' + ne]), hint: '想一想：1 到 ' + ne + ' 里偶数有几个，占全部的几分之几。' };
    }
    if (v === 'certain') {
      var cnt = rnd(3, 9);
      return { q: '盒子里有 ' + cnt + ' 个球，全是红球。任意摸出一个，摸到红球的可能性是（  ）', answer: '1', options: shuffle(['1', '1/2', '1/' + cnt, '0']), hint: '所有结果都是红球，是必然事件。' };
    }
    if (v === 'draw') {
      var d = rnd(2, 12);
      var digit = rnd(0, d);
      return { q: '从 0 到 ' + d + ' 这 ' + (d + 1) + ' 个数字中任意抽一个，抽到 ' + digit + ' 的可能性是（  ）', answer: '1/' + (d + 1), options: shuffle(['1/' + (d + 1), '1/' + d, '1/' + (d + 2), '1']), hint: (d + 1) + ' 个数字里指定的那个只有 1 个，占几分之几。' };
    }
    if (v === 'dice') {
      var face = rnd(1, 6);
      return { q: '掷一枚均匀的骰子，朝上的点数是 ' + face + ' 的可能性是（  ）', answer: '1/6', options: shuffle(['1/6', '1/3', '1/2', '1']), hint: '骰子有 6 个面，每个面朝上的可能性相等。' };
    }
    if (v === 'spinner') {
      var k = pick([3, 4, 5, 6, 8, 10]);
      return { q: '转盘被平均分成 ' + k + ' 等份，指针指向其中任意一份的可能性是（  ）', answer: '1/' + k, options: shuffle(['1/' + k, '1/' + (k - 1), '1/' + (k + 1), '1']), hint: k + ' 等份中指定一份占 1/' + k + '。' };
    }
    if (v === 'diceLt') {
      var ml = rnd(2, 6);
      return { q: '掷一枚均匀的骰子，朝上的点数小于 ' + ml + ' 的可能性是（  ）', answer: (ml - 1) + '/6', options: shuffle([(ml - 1) + '/6', (7 - ml) + '/6', '1/2', '1/6']), hint: '点数小于 ' + ml + ' 的有 1 到 ' + (ml - 1) + ' 共 ' + (ml - 1) + ' 种结果，占 6 份中的 ' + (ml - 1) + ' 份。' };
    }
    if (v === 'diceGt') {
      var mg = rnd(1, 5);
      return { q: '掷一枚均匀的骰子，朝上的点数大于 ' + mg + ' 的可能性是（  ）', answer: (6 - mg) + '/6', options: shuffle([(6 - mg) + '/6', mg + '/6', '1/2', '1/6']), hint: '点数大于 ' + mg + ' 的有 ' + (mg + 1) + ' 到 6 共 ' + (6 - mg) + ' 种结果，占 6 份中的 ' + (6 - mg) + ' 份。' };
    }
    var r2 = rnd(2, 7), w2 = rnd(1, 6), b2 = rnd(1, 6);
    var tot3 = r2 + w2 + b2;
    var want = rnd(0, 2);
    var nm = want === 0 ? ('红') : (want === 1 ? '白' : '蓝');
    var cc = want === 0 ? r2 : (want === 1 ? w2 : b2);
    return { q: '袋子里有 ' + r2 + ' 个红球、' + w2 + ' 个白球和 ' + b2 + ' 个蓝球，任意摸出一个，摸到' + nm + '球的可能性是（  ）', answer: cc + '/' + tot3, options: shuffle([cc + '/' + tot3, (tot3 - cc) + '/' + tot3, '1/' + tot3, '1']), hint: '可能性 = ' + nm + '球个数 ÷ 球的总数 = ' + cc + ' ÷ ' + tot3 + '。' };
  }
