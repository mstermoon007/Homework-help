...
(function (global) {
  ...
  function renderMultiTable(sc, grid, blanks) {
    ...
    html += '</tr><tr>';
    sc.cols.forEach(function (cn, c) {
      html += '<th style="border:1px solid #c9d4e6;padding:2px 10px;color:#7a879c;font-weight:400;font-size:11px;">' + (c + 1) + '</th>';
    });
    html += '</tr>';
    // 数据行
  ...
  function renderStatCard(p, i) {
    ...
    if (p.inputType === 'multi') {
      ...
      p.blanks.forEach(function (label, j) {
        blanksHTML += '<input type="text" class="answer-inp" data-idx="' + i + '" data-field="' + j + '" placeholder="?" autocomplete="off" style="width:52px;height:32px;border:2px dashed #ccc;border-radius:7px;font-size:15px;font-weight:700;text-align:center;color:#3f6fd1;background:#fafafa;outline:none;margin:0 4px;">' + '<span style="font-size:12px;color:#7a879c;">' + label + '</span>';
      });
      ...
      inputHTML = '<div class="input-group" style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:6px;">' +
        ...
        '<span style="font-size:13px;color:#7a879c;font-weight:600;">票</span>' +
        '</div>';
    }
