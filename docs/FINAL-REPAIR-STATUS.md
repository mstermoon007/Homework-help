...
| FINAL-135 | DEF-008 后半：nginx server_tokens off——关闭 Server 响应头与默认错误页中的版本号/发行版泄露（http 块全局，覆盖域名站与裸 IP default_server） | VERIFIED | 最小修改：nginx.conf http 块第 21 行官方预留 `# server_tokens off;` 取消注释（nginx -T 确认全配置树仅此 1 处生效，无重复指令），nginx -t + reload。实测 4 入口（HTTPS 首页/practice、HTTP 80 跳转、裸 IP）Server 头全部仅 `nginx`；3 错误页（HTTPS 404、/stats 401、裸 IP 404）页脚仅 `nginx`，响应体版本/Ubuntu 字符串计数 0；功能回归主站 200、HTTP 301、stats 401/200、HSTS max-age=300 与 LE 证书均不受影响。备份 /root/nginx.conf.bak-20260925-final135。DEF-008 两项（HSTS+server_tokens）全部闭合转 FIXED。详见 change-log FINAL-135 |
| FINAL-134 | FINAL-132 统计报表中文化 + 累计访客数：GoAccess 报表页面整体译为中文，顶部新增累计访客/累计请求/统计区间横幅 | VERIFIED | 纯服务器侧（零产品源码/零发布包）。①`--html-report-title='小学练习本 · 访问统计'`；②新增 /var/lib/goaccess/zh-custom-base.js（IIFE：全词精确字典替换 UI 文案 + MutationObserver 应对动态渲染 + 碎片日期/轴刻度/状态码分类/副标题/页脚专项处理，URL/IP/品牌名/攻击流量二进制等数据零误伤）；③make_meta.py 每次从 JSON 报表 general.unique_visitors/total_requests/start-end 烘焙数字到 /var/www-stats/zh-custom.js（goaccess 1.8.1 的 --html-custom-js 输出为相对 script 引用而非内联，故脚本与报表同目录并经现有 Basic Auth+noindex 保护）；④刷新脚本三步（JSON→meta+base→HTML），旧脚本备份 /root/goaccess-report.sh.bak-20260925-final134。headless Chrome 20s 虚拟时间完整渲染终验：可见区英文 UI 残留 0、横幅在线（当前 1,993 人/22,283 次/2026-09-11～25）、图表/表头/卡片/副标题/页脚全中文、数据值保持原样；线上 401/200/noindex 复验通过；cron :37 不变。详见 change-log FINAL-134 |
| FINAL-133 | DEF-008 前半：启用 HSTS（Strict-Transport-Security，max-age=300 小值起步，观察后再提长；不含 server_tokens，那半项保留 OPEN） | VERIFIED | 443 server 块 index 行后加 1 行 `add_header ... "max-age=300" always;`，nginx -t + reload。实测 7 个 HTTPS 响应（/、practice、select、知识页、bundle、sitemap、404）全带 HSTS；HTTP 301 不带（正确）；/stats 因 location 自身 add_header 不继承（X-Robots-Tag 保留，预期内，HSTS 按整主机记忆由主页覆盖）；stats 401 与主站 200 正常。备份 /root/nginx-home.modouyu.top.bak-20260925-final133。提升路线：观察无异常后 max-age 300→86400→31536000（未来另开任务，含 includeSubDomains/preload 再评估）。详见 change-log FINAL-133 |
| FINAL-132 | 浏览量统计小工具：nginx 访问日志 + GoAccess 定时静态报表（/stats/，Basic Auth 保护，X-Robots-Tag noindex），覆盖全部 381 页面，零产品源码/零发布包改动 | VERIFIED | 服务器侧新增：apt goaccess 1.8.1 + apache2-utils；/var/lib/goaccess 持久化 DB（zcat access.log* 全量喂入，重解析数字指纹一致=不重复计数）；/var/www-stats/index.html 926K 自包含报表（8 面板，9/11–9/25 共 15 天）；/usr/local/sbin/goaccess-report.sh 原子刷新 + /etc/cron.d/goaccess 每小时 :37（cron active）；443 块 include snippets/stats-goaccess.conf（alias webroot 外目录 + auth_basic + X-Robots-Tag）。实测：无/错口令 401、正确 200、/stats 301、HTTP→HTTPS 301、裸 IP 404 隔离、主站 200 不受影响。凭据 admin/随机 16 位（仅服务器 /etc/nginx/.htpasswd-stats 与用户手中）。配置备份 /root/nginx-home.modouyu.top.bak-20260925-final132。DEF-008 登记（HSTS+server_tokens 加固批，用户已约定稍后）。详见 change-log FINAL-132 |
| FINAL-131 | 修复 DEF-007：生产域名启用 HTTPS（Let's Encrypt + nginx 443 + HTTP 强制跳转），SW 在正式域名恢复注册条件 | VERIFIED | 零仓库源码/零发布包改动（sitemap/robots 本就是 https，产品无 http 外链）。certbot 2.9.0 签发 CN=home.modouyu.top（Let's Encrypt YE1，TLSv1.3，有效期至 2026-12-24），certbot 自动改 nginx（443 ssl + 80 301 跳转），443 外部可达，HTTP→301→HTTPS 实测，HTTPS 11 关键路径 200，旧文件 404，裸 IP HTTP 行为不变；certbot renew --dry-run 成功；真实浏览器 secureContext=true、生成 20 题、刷新后 SW controller=true + cache hw-help-5.0.0、零混合内容。nginx 原配置备份 /root/nginx-home.modouyu.top.bak-20260925。DEF-007 转 FIXED。详见 change-log FINAL-131 |
| FINAL-130 | 服务器上传上线：备份旧版→scp→原子替换→线上验证（本专项最后动作，禁止上传后发现再改循环） | VERIFIED | 目标 /var/www/Homework-help（nginx home.modouyu.top + default_server，仅 80 端口）。旧版（8/21 多学科 126 文件含 .git）备份至 /root/Homework-help.bak-20260925.tar.gz 后全量替换为 5.0.0 白名单包（571 文件，SHA256=d0a66e98...，源自 commit 56b643a）。验证：scp 后服务器端 hash 逐字节一致；curl 13 关键路径全 200；旧版文件 8 项全 404 无混合态；live hash 抽查 4/4 与 git HEAD MATCH；真实浏览器 首页→select→生成 19 题→打印 全 PASS；FINAL-123 修复后 compat 404 已消除（DOM 级确认）。详见 change-log FINAL-130 |
| FINAL-123 | 清除 FINAL-22 删 knowledge-compat.js 时遗留的 2 处死引用：select.html script 标签 + sw.js CORE 预缓存条目 | VERIFIED | 删 2 行零逻辑；check-all 双跑 28/0/0 一致 + git diff 零增长；commit 56b643a；重新出包 571 文件 SHA256=d0a66e98...；离线重验证 9 路径全 200 + compat 按预期 404；线上复验 compat 引用=0。详见 change-log FINAL-123 |
| FINAL-122 | 发布包再次离线验证：干净解压+本地静态服务器走 首页→知识页→练习→生成→打印 | VERIFIED | tar.gz 解压 571 文件到 /tmp 干净目录，python http.server 根部署，真实浏览器全链 5 步 PASS：①首页（title/logo/5.0.0/备案号/零 404）②知识页（tokens.css 200、相对路径 CTA）③练习入口（两个 bundle 200、10 个 KBL JSON 全 200）④生成（单题型 + 7 题型 21 题覆盖 calc/fill/choice/judge/shape/sort/story 全 7 类）⑤打印（window.open 钩子 opens/writes/prints=1，htmlLen 24615，hasCards=true，不触发生成）。全程无 console error、包内零 404。空配置提示"无可生成题目"为 fail-closed 正常行为。结论：构建/压缩/路径变化未造成故障。详见 change-log FINAL-122 |
| FINAL-121 | 发布包内容检查：白名单产品包，禁止项全 0 | VERIFIED | 沿入口引用链（7 根页 script + sw.js 缓存清单 + knowledge-runtime 数据路径）定白名单，从 commit bf7f625 用 git archive pathspec 重建：3.2M/571 文件，SHA256=5335c2c6...。必含全齐：HTML 384（7 根+376 knowledge+feedback）、JS 153、CSS 8、SVG 插件 6、assets 6、sitemap/robots/llms/CNAME/VERSION/LICENSE/sw.js、KBL 运行时 manifest、两个最终 bundle。禁止项全 0：tests/dev/archive/migration/scripts/tools/docs/architecture/audit-results/.github/node_modules/.trae、gitignore/package.json/P16/svg-test、report|audit、.env、旧 bundle、kbl 源目录、bak/old/tmp/DS_Store。解包抽查 3 文件 hash 与 git show HEAD 一致证来源。详见 change-log FINAL-121 |
| FINAL-120 | 生成正式发布包：来自冻结 commit，排除工作目录/本地临时文件 | VERIFIED | `git archive` 从 commit 9feccd6 生成 release/homework-help-5.0.0.tar.gz（5.3M），SHA256=53e90188...。源码冻结基线 4123124（git diff 源码为空）。禁止项全 0：node_modules/.trae/dev-reports/dist/.DS_Store/release 均 0。关键文件齐全（VERSION/页面/bundle/KBL/FINAL-FREEZE.md）。清理了误跟踪的 .trae 本地文档，.gitignore 新增 release/。发布清单 release/RELEASE-MANIFEST-5.0.0.md。详见 change-log FINAL-120 |
